...
describe Dog do
  ...
  subject(:azor) { described_class.new(name: 'Azor', breed: "Wilczur", color: 'brown') }
  # Write a test using the second shared_example to test that dog responds to
  # talk ('WOOF!').
  context 'when Dog has the ability to talk' do
    include_examples "shared method name"
  end

  # remove the 'x' before running this test
  it 'is not barking' do
    expect(azor).not_to be_barking
  end

  # remove the 'x' before running this test
  it 'is sleeping' do
    expect(azor).to be_sleeping
  end
end
