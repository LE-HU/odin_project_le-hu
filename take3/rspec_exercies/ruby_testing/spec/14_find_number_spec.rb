...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 0 and max is 9' do
      it 'returns 4' do
        expect(game_guessing.make_guess).to eq((game_guessing.min + game_guessing.max)/2)
      end
    end
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:rand_number) { double('rand_number', value: 7) }
      subject(:game_finishing) { described_class.new(0, 9, rand_number, 7) }
      ...
      it 'is game over' do
        expect(game_finishing).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      let(:rand_number) { double('rand_number', value: 7) }
      subject(:game_finishing) { described_class.new(0, 9, rand_number, 5) }
      it 'is not game over' do
        expect(game_finishing).not_to be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    let(:number_range) { double('random_number', value: 7) }
    ...
    context 'when the guess is less than the answer' do
      ...
      it 'updates min to 5' do
        expect { low_guess_game.update_range }.to change { low_guess_game.min }.to(1)
